    ...
    if doc.work_order:
        ...
        for i in doc.locations:
            ...
            if len(woi) > 0:
                ...
                frappe.db.sql(""" UPDATE `tabWork Order` SET material_transferred_for_manufacturing=%s, qty=%s WHERE name=%s """,(wo.material_transferred_for_manufacturing + 1,wo.qty + 1,doc.work_order))
                frappe.db.commit()
