...
def get_mr(supplier, mr):
    ...
    for x in data:
        mr = frappe.db.sql(""" SELECT item_code, item_name, description, uom,conversion_factor, stock_uom, qty,budget_bom_rate,rate, amount FROM `tabMaterial Request Item` MRI WHERE MRI.parent=%s """, x,as_dict=1)
        for i in mr:
