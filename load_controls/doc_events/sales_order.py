...
def on_submit_dn(doc, method):
    ...
    for i in doc.budget_bom_references:

        po = frappe.db.sql(""" SELECT COUNT(*) as count FROM `tabSales Order` Where budget_bom=%s and docstatus=1 """, i.budget_bom, as_dict=1)
        pi_query = """ SELECT COUNT(*) as count FROM `tab{0}` DD INNER JOIN  `tabBudget BOM References` BBR ON BBR.parent = DD.name and BBR.buget_bom = '{1}' and DD.docstatus=1""".format(doctype, i.budget_bom)
